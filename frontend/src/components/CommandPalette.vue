  ...
  <Teleport to="body">
    <Transition name="fade">
      <div v-if="isOpen" class="fixed inset-0 z-[200] flex items-start justify-center pt-20 sm:pt-32 px-4 pb-20 bg-slate-900/40 dark:bg-[#0B1120]/60 backdrop-blur-sm" @click="emit('close')">
        <div class="w-full max-w-2xl bg-white/70 dark:bg-[#0f172a]/70 backdrop-blur-2xl backdrop-saturate-200 rounded-2xl shadow-2xl overflow-hidden border border-border/50 relative" @click.stop>
          ...
          <!-- Search Input -->
          <div class="relative z-20 flex items-center px-4 py-4 border-b border-border">
            <Search class="w-6 h-6 text-slate-400 shrink-0" />
            <input 
              ref="searchInput"
              v-model="query" 
              type="text" 
              class="w-full bg-transparent border-none px-4 text-lg text-foreground placeholder:text-slate-400 focus:ring-0 focus:outline-none" 
              placeholder="搜索应用名称、开发者、关键词..."
              @keydown.esc="emit('close')"
            />
            <button @click="emit('close')" class="p-1 rounded-md text-muted-foreground hover:bg-accent transition-colors">
              <X class="w-5 h-5" />
            </button>
          </div>
          ...
          <div class="relative z-20 max-h-[60vh] overflow-y-auto p-2">
            ...
            <div v-if="query.trim() === ''" class="px-4 py-12 text-center text-slate-500">
              ...
              <div class="flex items-center justify-center gap-2 mt-4 text-xs">
                <kbd class="px-2 py-1 bg-secondary rounded border border-border">ESC</kbd> 退出
              </div>
            </div>

            <div v-else-if="searchResults.length === 0" class="px-4 py-12 text-center text-muted-foreground">
            ...
            <div v-else class="space-y-1">
              ...
              <button 
                ...
                class="w-full flex items-center gap-4 px-3 py-3 rounded-xl hover:bg-accent transition-colors text-left group"
              >
          ...
          <div class="px-4 py-3 bg-secondary border-t border-border flex items-center justify-between text-xs text-muted-foreground">
