<template>
  <div class="p-4 lg:p-6 rounded-2xl bg-card border border-border">
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8">
      <div>
        <label class="block text-sm font-bold text-foreground mb-4 flex justify-between items-center">
          <span>应用图标 (Icon)</span>
        ...
        <div class="flex gap-4 items-center">
          <div class="w-16 h-16 shrink-0 bg-card rounded-xl shadow-sm border border-border flex items-center justify-center p-2">
            <img v-if="icon" :src="icon" class="w-full h-full object-contain" alt="" />
            <span v-else class="text-muted-foreground text-xs">无图</span>
          </div>
          <input
            type="text"
            :value="icon"
            @input="icon = normalizeMediaUrl(($event.target as HTMLInputElement).value)"
            class="flex-1 px-4 py-3 lg:py-2 rounded-xl border border-border bg-card outline-none focus:border-blue-500 text-base"
      ...
      <div>
        ...
        <div class="flex flex-col gap-3">
          <div class="w-full h-24 bg-card rounded-xl shadow-sm border border-border flex items-center justify-center p-1 overflow-hidden">
            <img v-if="banner" :src="banner" class="w-full h-full object-cover rounded-lg" alt="" />
            <span v-else class="text-muted-foreground text-xs">无横幅</span>
          </div>
          <input
            type="text"
            :value="banner"
            @input="banner = normalizeMediaUrl(($event.target as HTMLInputElement).value)"
            class="w-full px-4 py-3 lg:py-2 rounded-xl border border-border bg-card outline-none focus:border-blue-500 text-base"
            placeholder="请上传并使用站内地址（/api/uploads/...）"
          />
        </div>
      </div>
