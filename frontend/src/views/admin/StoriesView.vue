<template>
  <div class="grid grid-cols-1 lg:grid-cols-4 gap-4 lg:gap-8">
    ...
    <div class="lg:col-span-3 bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden flex flex-col" :class="{ 'hidden': isMobile && mobileView === 'list' }" v-if="currentStory">
      ...
      <!-- 移动端底部操作栏 -->
      <div v-if="isMobile" class="p-4 border-t border-slate-100 dark:border-slate-700 bg-white dark:bg-slate-800">
        <button @click="saveStories" class="w-full px-4 py-3 rounded-2xl bg-emerald-500 hover:bg-emerald-600 text-white font-bold transition-colors flex items-center justify-center gap-2">
          <Save class="w-4 h-4" />
          {{ isSaving ? '保存中...' : '保存文章' }}
        </button>
      </div>
