    ...
    <template v-else-if="org">
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-3">
          <div class="w-10 h-10 rounded-xl overflow-hidden flex-shrink-0 bg-slate-100 dark:bg-slate-700">
            <img v-if="org.avatar_url" :src="org.avatar_url" :alt="org.name" class="w-full h-full object-cover" />
        ...
        <button v-if="org.is_admin" @click="saveOrg" :disabled="isSaving" class="px-4 py-2 rounded-xl bg-blue-500 hover:bg-blue-600 text-white text-sm font-bold shadow-lg shadow-blue-500/20 transition-all disabled:opacity-50">
          {{ isSaving ? '保存中...' : '保存修改' }}
        </button>
      </div>

      <div v-if="org.is_admin" class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
      ...
      <div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
        <div class="p-4 lg:p-6 border-b border-slate-100 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-900/50 flex items-center justify-between">
          ...
          <button v-if="org.is_admin" @click="showInviteMember = true" class="px-3 py-1.5 rounded-lg bg-blue-500 hover:bg-blue-600 text-white text-xs font-bold transition-colors">
            邀请成员
          </button>
        </div>
          ...
          <div class="space-y-2">
            <div v-for="m in members" :key="m.user_id" class="flex items-center gap-3 p-3 rounded-xl bg-slate-50 dark:bg-slate-900/50">
              <div class="w-8 h-8 rounded-full overflow-hidden bg-slate-200 dark:bg-slate-700 flex-shrink-0">
                <div class="w-full h-full flex items-center justify-center text-xs font-bold text-slate-400">{{ (m.user_name || '?')[0] }}</div>
              </div>
              ...
              <select
                v-if="org.is_admin && m.role !== 'owner'"
                :value="m.role"
...
const fetchOrg = async () => {
  ...
  try {
    ...
    if (res.ok) {
      ...
      org.value = json;
      draft.value = { name: json.name, description: json.description, website_url: json.website_url };
