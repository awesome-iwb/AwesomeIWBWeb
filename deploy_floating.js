...
conn.on('ready', () => {
  ...
  conn.sftp((err, sftp) => {
    ...
    function uploadNext() {
      ...
      writeStream.on('error', (e) => { console.error(`  ✗ ${relPath}:`, e.message); idx++; uploadNext(); });
      readStream.pipe(writeStream);
    }

    function buildAndDeploy() {
      const commands = [
        `cd /opt/awesomeiwb && git add -A && git commit -m "sync" --allow-empty 2>/dev/null; echo "git synced"`,
